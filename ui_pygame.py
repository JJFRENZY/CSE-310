...
# Battle log lines as either plain string or a special "turn header" tuple
LogLine = Union[str, Tuple[str, int, Command, Command]]  # ("TURN", idx, p_cmd, c_cmd)
    ...
    while True:
        ...
        if mode == "select":
            ...
            if mode == "result":
                ...
                winner_color = ACCENT
                if battle_winner == player.name:
                    winner_color = p_color
            ...
            y = log_panel.y + 14
            for entry in view:
                if isinstance(entry, tuple) and entry[0] == "TURN":
                    _, t_idx, p_cmd, c_cmd = entry
                    draw_inline_segments(
                        screen, font,
                        [
                            (f"Turn {t_idx}: ", TEXT),
                            (f"{player.name}", p_color),
                            (f"[{p_cmd.value}] ", TEXT),
                            ("vs ", TEXT),
                            (f"{cpu.name}", c_color),
                            (f"[{c_cmd.value}]", TEXT),
